...
### 功能说明
沙箱在初始化时自动检测并配置 matplotlib 以支持中文字符显示，解决图表中中文字段乱码的问题。

### 跨平台字体策略
系统会根据操作系统自动选择合适的中文字体：
...
### 实现细节
- 字体检测函数：`_detect_chinese_font()` in `sandbox.py`
- 配置方法：`PythonSandbox._configure_matplotlib_fonts()`
- 自动配置 `rcParams['font.sans-serif']` 和 `rcParams['axes.unicode_minus']`
