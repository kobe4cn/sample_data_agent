...
- 🧼 **智能数据预处理** - 在分析前检查 Excel/CSV 结构、清理列名、验证列是否存在，详见 [DATA_PREPARATION.md](backend/DATA_PREPARATION.md)
...
| `lego` | Excel（多层表头，已预清洗） | 中等规模 | LEGO 产品销售数据 |
...
# 1. 直接加载预清洗的 LEGO 数据
lego_df = load_dataset('lego')
print(lego_df.columns[:8])  # ['人群', '系列', '孩子_人群交Passion曝光base', ...]

# 2. 处理自定义多层表头 Excel
custom_df = load_multiheader_excel(
    'data/custom_report.xlsx',
    header_row=3,            # 真实表头所在行（0-index）
    depth=2,                 # 表头占用的层数
)
```

> `load_multiheader_excel` 会自动跳过元信息行、删除 `Unnamed` 列并扁平化列名。如需保留特殊列，可设置 `drop_unnamed=False` 或通过 `DatasetConfig` 自定义更多行为。
...
- **官方指南**：阅读 [DATA_PREPARATION.md](backend/DATA_PREPARATION.md) 获取多层表头、列名清理、类型转换和宽表转长表的完整流程。
- **练习数据**：仓库内置 `data/test_multiheader.xlsx` 与 `data/test_wide.csv`，可直接在 `python_inter` 中练习“header=None → header=3/skiprows → melt”流程。
- **自动化校验**：运行 `cd backend && ../.venv/bin/python -m unittest tests.test_data_preprocessing` 验证 LEGO、多层表头、宽表转长表以及 telco 回归测试均通过。
- **AI 互动模板**：参考指南中的“一分钟上手”章节，将列名列表、结构截图贴回给 AI，可避免 `KeyError: 'passion_系列'` 等问题。
